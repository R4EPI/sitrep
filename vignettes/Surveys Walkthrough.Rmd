...
**This vignette will demonstrate using *data collected with Kobo***. The data are stored in the {sitrep} R package and can be accessed by running these commands:
...
Because these datasets are already stored within the {sitrep} R package, once that package is loaded, the commands below will open the datasets in your RStudio environment for analysis.

```{r}
# access the survey responses for this vignette and store in your R Environment for use
survey_data_ind <- survey_data   # the survey responses
survey_data_hh <- survey_hhdata  # responses from household members
```
...
Surveys conducted with Kobo produce a data dictionary file, in which information about the variables are stored in one sheet, and informaton about the values ("choices") are stored in an adjacent sheet in the same Excel workbook. 

Both of these have been stored with the {sitrep} R package and are easy to bring into your RStudio environment with the following commands.  

```{r, echo=TRUE}
# access the kobo data dictionary tables
kobo_vars <- kobo_vars     # the variable names and labels
kobo_values <- kobo_values # the survey choices and labels
```

Successfully running the above commands should result in 4 data frame objects in your RStudio Environment pane, as shown below:  

![](images/mort_import.png){width="549"}

You can click on the names of these files to open them in a Viewer pane.

The Kobo data dictionary of the survey variables should look like this (first 25 rows):  

```{r, echo=FALSE}
DT::datatable(head(kobo_vars, 25), rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```

</br>

...and the first 25 rows of the data dictionary on the survey question values should look like this:  

```{r, echo=FALSE}
DT::datatable(head(kobo_values, 25), rownames = FALSE, filter="top", options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```
...
In your real-life situations, you will store the survey results and the data dictionary (e.g. .xlsx or .csv) in the RStudio project folder, and import them using `import()` and `here()`, as described in the Epidemiologist R Handbook's [Import and export](https://epirhandbook.com/en/import-and-export.html) chapter.
...
Th example command below imports an Excel spreadsheet called "kobo_dictionary.xlsx" which is not password protected, and is stored in a "survey" subfolder of the RStudio project. The `which =` argument of `import()` specifies that the sheet "variables" should be imported. You would edit this command as appropriate for your situation.

```{r echo=TRUE, eval=FALSE}
# example code for importing data from an Excel file saved in RStudio project
# This imports a specific sheet, named "Sheet1"
linelist_raw <- import(here("survey", "kobo_dictionary.xlsx"), which = "variables", na = "")
```
...
```{r}
a <- msf_dict_survey(name = system.file("extdata", "_gen_KoboMortalitySurvey.xls", package = "sitrep"),
                     template = FALSE)
```


```{r, eval=FALSE}
msf_dict <- msf_dict_survey(name = "C:/Users/neale/Documents/Applied Epi/repos/sitrep/inst/extdata/_gen_KoboMortalitySurvey.xls", template = FALSE)
```
