...
This page demonstrate use of the {sitrep} R Markdown template for analysis of a retrospective mortality survey, using data that was produced using Kobo, a common survey and data collection tool in settings where MSF works.
...
In the cluster counts chunk, we specify that Town A has 500 households, whereas Town B has 700.  
...
Running some summary statistics on these data show that most rows were not able to be calculated:  

```{r}
summary(study_data_cleaned$obstime)
```

For row that had data, this is the distribution of `obstime`:  

```{r}
ggplot(study_data_cleaned, aes(x = obstime))+
  geom_histogram()
```
...
In the command below, we create the object `var_labels`, using the `setNames()` function (from the ackage {stats}). The "short names" from the data dictionary are set as the display labels, while the current variables names are retained.  
...
Now we use this object `var_labels` within the function `set_variable_labels()`, to set the labels of our dataset.  
...
Lastly, an option is shown for how to manually adjust column labels (if you did not want to use the data dictionary)


### Remove unused data  

This chunk contains code to drop and examine rows for which there is no consent, there are missing start or end dates, village_name is "Other", age is missing, or sex is missing. These rows are first saved as `dropped`. This allows you to inspect them before actually dropping the rows.
...
Its a cluster survey but because we have poip counts for age and sex, we are able to adjust to reflect the population better and produce stratefied estimate by age and sex. 
...
survey design- delete the others and just run strat cluster
