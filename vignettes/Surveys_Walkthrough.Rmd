...
population_data_age_district_a <- gen_population(total_pop = 10000, # set total population 
  ...
  rename(age_group  = groups,                      # rename columns (NEW NAME = OLD NAME)
         sex        = strata,
         population = n) %>% 
  mutate(village_name = "Town A")           # add a column to identify region 
...
population_data_age_district_b <- gen_population(total_pop = 10000, # set total population 
  ...
  rename(age_group  = groups,                      # rename columns (NEW NAME = OLD NAME)
         sex        = strata,
         population = n) %>% 
  mutate(village_name = "Town B")           # add a column to identify region 
...
```{r}
study_data_cleaned %>% 
  tabyl(cluster_number, village_name)
```

<!-- We can also see from the ordering in the table above, that cluster number is recognized as a character and not as a number, so we fix this with a quick `mutate()` command:   -->
...
study_data_cleaned <- study_data_cleaned %>% 
  mutate(cluster_number_uid = str_glue(
    ...
    town_letter = str_sub(village_name, -1)))
```
...
```{r}
study_data_cleaned %>% 
  tabyl(cluster_number_uid, village_name)
```
...
dropped <- study_data_cleaned %>% 
  filter(!consent | is.na(startdate) | is.na(enddate) | 
           ...
           is.na(village_name) | is.na(age_years) | is.na(sex))
...
At the end of the first command, we remove "health_district" because we only want to stratify by age and sex.  
...
* We change `health_district` to `village_name`, as this is our strata
...
survey_design <- study_data_cleaned %>% 
  as_survey_design(ids = cluster_number_uid, # cluster ids 
                   ...
                   strata = village_name # sampling was stratified by district
                  )
...
Replace `health_district` with `village_name`.  
...
Below, again, we replace `health_district` with `village_name`

```{r CMR_strata, warning = FALSE, message = FALSE}

## weighted counts and proportion of dead
death_props_strat <- survey_design %>% 
  select(died, village_name) %>%
  tbl_svysummary(by = village_name) %>% 
  ## add the weighted total
  add_n() %>%   
  ## add in confidence intervals
...
We also edit the in-line code (dynamic text) so that any reference to `health_district` changes to `village_name`, and "district_a" and "district_b" are changed to "Town A", and "Town B".  
...
<!-- `r CMR_strat %>% filter(village_name == "Town A") %>% pull(Mortality)` -->
<!-- deaths per 10000 person-days and in District B was  -->
<!-- `r CMR_strat %>% filter(village_name == "Town B") %>% pull(Mortality)` -->
