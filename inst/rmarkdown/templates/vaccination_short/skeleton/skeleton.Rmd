---
title: "Vaccination survey short"
output: 
  word_document:
    keep_md: true
---
...
coverage survey from the short dictionary. 
...
study_data_dict <- msf_dict_survey("Vaccination_short")

## generates MSF standard dictionary for Kobo in long format (for recoding)
study_data_dict_long <- msf_dict_survey(disease = "Vaccination_short", compact = FALSE)
...
study_data_raw <- gen_data(dictionary = "Vaccination_short", 
                           varnames = "name",
                           numcases = 1000)
```
...
# msf_dict_rename_helper("vaccination_short", varnames = "name")
...
study_data_cleaned <- study_data_cleaned %>% 
  mutate(vaccination_status = case_when(
    ...
    msf_vacc     == "Yes, card"             ~ "Card", 
    routine_vacc == "Yes, card"             ~ "Card", 
    sia_vacc     == "Yes, card"             ~ "Card", 
...
study_data_cleaned <- study_data_cleaned %>% 
  set_variable_labels(
    ...
    time_since_group = "Time since diagnosis (years)"
    )
```
...
survey_design %>% 
  select(vaccination_status_simple) %>% 
  tbl_svysummary() %>% 
...
survey_design %>% 
  select(vaccination_status_simple, health_district) %>%
...
The below chunk creates a weighted table of counts proportions for vaccination 
coverage while accepting equal validity from self- and card-reporting; broken 
down by sex. 
...
survey_design %>% 
  select(vaccination_status_simple, sex) %>%
  ...
  modify_header(
    list(
      n ~ "**Weighted total (N)**",
      stat_1 ~ "**Male Weighted Count (n)** \n N={n}", 
      stat_2 ~ "**Female Weighted Count (n)** \n N={n}" 
    )
    )
```
...
survey_design %>% 
  select(vaccination_status_simple, age_group) %>%
  ## Note that here we swap the stratifier 
  ## this way age group becomes the rows, so we need to calculate percent by row
  tbl_svysummary(by = vaccination_status_simple, 
                 percent = "row") %>% 
  ## add the weighted total
  add_n() %>%   
  ## add in confidence intervals
  ...
  modify_header(
    list(
      n ~ "**Weighted total (N)**",
      stat_1 ~ "**Vaccinated Weighted Count (n)** \n N={n}", 
      stat_2 ~ "**Unvaccinated Weighted Count (n)** \n N={n}" 
    )
    )
```
...
survey_design %>% 
  select(vaccination_status, age_group) %>%
  ## Note that here we swap the stratifier 
  ## this way age group becomes the rows, so we need to calculate percent by row
  tbl_svysummary(by = vaccination_status, 
                 percent = "row") %>% 
  ## add the weighted total
  add_n() %>%   
  ## add in confidence intervals
  ...
  modify_header(
    list(
      n ~ "**Weighted total (N)**",
      stat_1 ~ "**Card Weighted Count (n)** \n N={n}", 
      stat_2 ~ "**Verbal Weighted Count (n)** \n N={n}", 
      stat_3 ~ "**Unvaccinated Weighted Count (n)** \n N={n}" 
    )
    )
```
...
Weighted counts and proportions for reasons not vaccinated in routine circumstances, individually, among those not vaccinated
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// no_vacc_reason_routine \\\
--------------------------------------------------------------------------------

The below chunk creates a weighted table of counts proportions for reasons 
not vaccinated during routine immunisation activitites. 
...
```{r no_vacc_reason_routine}

survey_design %>% 
  ## filter for those not vaccinated 
  filter(!routine_vacc %in% c("Yes, card", "Yes, verbal")) %>%
  select(reason_route_vacc) %>% 
  tbl_svysummary() %>% 
  ## add in confidence intervals
...
Weighted counts and proportions for reasons not vaccinated in campaign circumstances, individually, among those not vaccinated
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// no_vacc_reason_campaign_sia_combo \\\
--------------------------------------------------------------------------------

The below chunk creates a weighted table of counts proportions for reasons 
not vaccinated during campaign immunisation activitites. 
...
```{r no_vacc_reason_campaign}

## campaign reasons 
survey_design %>% 
  ## filter for not vaccinated 
  filter(!msf_vacc %in% c("Yes, card", "Yes, verbal")) %>%
  select(reason_msf_vacc) %>% 
  tbl_svysummary() %>% 
  ## add in confidence intervals
  ...
  modify_header(
    list(
      stat_0 ~ "**Campaign Weighted Count (n) \n (N={round(N, digits = 0)})**"
    )
    )
```
...
survey_design %>% 
  ## filter for not vaccinated 
  filter(!sia_vacc %in% c("Yes, card", "Yes, verbal")) %>%
  select(reason_sia_vacc) %>% 
  tbl_svysummary() %>% 
  ## add in confidence intervals
  ...
  modify_header(
    list(
      stat_0 ~ "**Supplementary Weighted Count (n) \n (N={round(N, digits = 0)})**"
    )
    )
```
