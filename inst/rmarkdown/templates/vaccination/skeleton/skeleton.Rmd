...
ag <- descriptive(study_data_cleaned, "age_group") %>%
  filter(age_group != "Missing") %>%  # drop missings to fit with population data
...
descriptive(dropped,
            "q65_consent_no_reason", 
            coltotals = TRUE) %>% 
  # change column names 
  rename(Reason = q65_consent_no_reason, 
         "Individuals (n)" = n, 
         "%" = prop)
```
...
survey_design %>%  # use the survey object (weighted)
  ...
  tabulate_survey(vaccination_status_simple, deff = TRUE, digits = 2) %>% 
  rename("Vaccination status" = vaccination_status_simple, 
         "Children (n)" = n, 
         "Design effect" = deff, 
         "% (95% CI)" = ci) %>% 
  kable(digits = 2)
...
survey_design %>% # use the survey object (weighted)
  # calculate weighted counts and weighted proportions, 
  # include design effect because cluster sample
  tabulate_survey(vaccination_status, deff = TRUE, digits = 2) %>%
  rename("Vaccination status" = vaccination_status, 
         "Children (n)" = n, 
         "Design effect" = deff, 
         "% (95% CI)" = ci) %>% 
  kable(digits = 2)
...
survey_design %>% # use the survey object (weighted)
  ...
  # include design effect because cluster sample
  tabulate_survey(vaccination_status_simple, sex, deff = TRUE, digits = 2) %>%
  rename("Vaccination status" = vaccination_status_simple) %>% 
...
# return weighted counts and weighted proportions 
survey_design %>% 
  tabulate_survey(vaccination_status, sex, deff = TRUE, digits = 2) %>%
  rename("Vaccination status" = vaccination_status) %>% 
...
# return weighted counts and weighted proportions 
survey_design %>% 
  tabulate_survey(vaccination_status_simple, age_group, deff = TRUE, digits = 2, wide = FALSE) %>%
  
  # spread values to wide format to have appropriate groups 
    # collect variables of interest (n, deff and ci) (call the clasifier column "variable")
  gather(key = "variable", value = "value", c(n, deff, ci)) %>% 
    # create temp column which combine the classifier column with vacc_status column seperated by underscore
  unite(col = "temp", vaccination_status_simple, variable, sep = "_") %>%
    # spread wide based on newly create combo column above 
  spread(temp, value) %>% 
  # change order of columns 
  select(age_group, Vaccinated_n, Vaccinated_ci, Vaccinated_deff, 
         Unvaccinated_n, Unvaccinated_ci, Unvaccinated_deff) %>% 
...
# return weighted counts and weighted proportions 
survey_design %>% 
  tabulate_survey(vaccination_status, age_group,  deff = TRUE, digits = 2, wide = FALSE) %>%
  
  # spread values to wide format to have appropriate groups 
    # collect variables of interest (n, deff and ci) (call the clasifier column "variable")
  gather(key = "variable", value = "value", c(n, deff, ci)) %>% 
    # create temp column which combine the classifier column with vacc_status column seperated by underscore
  unite(col = "temp", vaccination_status, variable, sep = "_") %>%
    # spread wide based on newly create combo column above 
  spread(temp, value) %>% 
  # change order of columns 
  select(age_group, Card_n, Card_ci, Card_deff, 
         Verbal_n, Verbal_ci, Verbal_deff,
         Unvaccinated_n, Unvaccinated_ci, Unvaccinated_deff) %>% 
...
routine <- survey_design %>% 
  ...
  tabulate_survey(q7_no_routine_vacc_r) %>%
  # rename columns appropriately
  rename(Reasons = q7_no_routine_vacc_r, 
         "Routine (n)" = n , 
         "% (95% CI)" = ci)
...
campaign <- survey_design %>% 
  ...
  tabulate_survey(q28_no_campaign_vacc_r) %>% 
  # drop the reasons column 
  select(-q28_no_campaign_vacc_r) 
...
survey_design %>% 
  ...
  tabulate_survey(q38_no_sia_vacc_reason) %>%
  # rename columns appropriately
  rename(Reasons = q38_no_sia_vacc_reason, 
         "Routine (n)" = n , 
         "% (95% CI)" = ci)
```
