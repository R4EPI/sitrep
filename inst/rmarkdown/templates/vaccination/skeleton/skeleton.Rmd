...
## In this example, we rename variables several variables
...
population_data_age_district_a <- gen_population(total_pop = 10000, # set total population 
  ...
  proportions = c(0.0329, 0.0329, 0.0310, 
                  0.0310, 0.0310, 0.2772), # set proportions for each group
...
study_data_raw %>% 
  filter(q56_routine_vaccine_age_mth > age_in_months) %>% 
  select("fact_0_id")
```
...
study_data_cleaned <- study_data_cleaned %>% 
  mutate(hh_id = glue::glue("{village}_{q14_hh_no}"))
...
iqr <- glue::glue_collapse(  # basically copy paste togehter the following
...
survey_design %>% 
  ...
  augment_redundant(" (n)"         = "_n") %>%  # wrap all "n" in braces (note space before n)
  rename_redundant("% (95% CI)"    = "_ci",       # relabel all columns containing "ci"
                   "Design Effect" = "_deff") %>% # "deff" to "Design effect"
  kable(digits = 1)
```
