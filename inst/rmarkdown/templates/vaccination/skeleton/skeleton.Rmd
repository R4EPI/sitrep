...
survey_design %>% # use the survey object (weighted)
  # calculate weighted counts and weighted proportions, 
  # include design effect because cluster sample
  tab_survey(vaccination_status, deff = TRUE, digits = 2) %>% 
  select(-variable) %>%
  rename("Vaccination status" = value, 
         "Children (n)" = n, 
         "Design effect" = deff, 
...
survey_design %>% # use the survey object (weighted)
  ...
  # include design effect because cluster sample
  tab_survey(vaccination_status_simple, strata = sex, deff = TRUE, digits = 2) %>%
  select(-variable) %>%
  rename("Vaccination status" = value) %>% 
...
# return weighted counts and weighted proportions 
survey_design %>% 
  tab_survey(vaccination_status, strata = sex, deff = TRUE, digits = 2) %>%
  select(-variable) %>%
  rename("Vaccination status" = value) %>% 
  augment_redundant(" (n)"         = " n") %>%    # wrap all "n" vars in braces (note space before n).
  rename_redundant("% (95% CI)"    = "ci",        # relabel all columns containing "ci" to "% (95% CI)"
...
# return weighted counts and weighted proportions 
survey_design %>% 
  tab_survey(vaccination_status, strata = age_group, deff = TRUE, digits = 2, 
             drop = "Missing", transpose = "value") %>%
  rename("Age group (years)" = age_group) %>% 
  augment_redundant(" (n)"         = "_n") %>%    # wrap all "n" vars in braces (note space before n).
  rename_redundant("% (95% CI)"    = "_ci",       # relabel all columns containing "ci" to "% (95% CI)"
