...
## select unique vaccines for use in iterating later
VACCINES <- CARDVARS %>% 
  ## using the list of vaccine names, remove numbers (for doses)
  str_remove("[:digit:]") %>% 
  ## keep only unique vaccines
  unique()
...
vacc_var_labels <- setNames(
  ## add variable labels as a list 
  as.list(study_data_dict$short_name[study_data_dict$type == "vaccine_card"]), 
...
CARDVARSDATES <- str_glue("date_{CARDVARS}")

counts <- survey_design %>% 
  ## only keep variables of interest
  group_by(date_dtp_hib1) %>%
  srvyr::survey_count(date_dtp_hib1)

ggplot(counts, aes(x = date_dtp_hib1, y = n)) + 
  ...
  scale_x_date(
    breaks = "1 month",
    date_labels = "%b %Y", 
    limits = c(as.Date("2018-01-01"), as.Date("2018-07-01"))) + 
  
  labs(y = "Weighted count", x = "Date of vaccination", 
       title = "date_dtp_hib1") +
...
survey_design %>% 
  ...
  filter(
    all_vaccines_survey_woc != "Yes" &
    all_vaccines_survey_card != "Yes") %>%
  select(reason_not_all_vacc) %>% 
  tbl_svysummary() %>% 
  ## add in confidence intervals
