...
coverage survey from the long dictionary. 
...
study_data_dict <- msf_dict_survey("Vaccination_long")

## generates MSF standard dictionary for Kobo in long format (for recoding)
study_data_dict_long <- msf_dict_survey(disease = "Vaccination_long", compact = FALSE)
...
study_data_raw <- gen_data(dictionary = "Vaccination_long", 
                           varnames = "name",
                           numcases = 1000)
```
...
#study_data_raw <- left_join(study_data_indiv, study_data_hh, by = c("_index" = "_parent_index"))
```
...
If you have already loaded data from Kobo, go to read_population_data.
...
# msf_dict_rename_helper("vaccination_long", varnames = "name")
...
study_data_cleaned <- study_data_cleaned %>%
  mutate(no_consent = fct_relevel(no_consent_reason,
                                  "There is no benefit to me and my family", 
                                  "I do not have time", 
                                  "I have had a negative experience with a previous survey.",
                                  "Other")
        )


## create simplified groupings for each of the vaccine status variables 
## simplified, TRUE (regardless of date) otherwise FALSE (or NA if missing) 

## select the variables from dictionary for vaccines by card 
CARDVARS <- study_data_dict %>% 
  filter(type == "vaccine_card") %>% 
  filter(name %in% names(study_data_cleaned)) %>% 
  ## filter to match the column names of your data
  pull(name) # select vars

## create simplified variables
study_data_cleaned <- study_data_cleaned %>% 
  mutate(
    ## for each of the vaccine variables
    dplyr::across(all_of(CARDVARS),
                  ## if a row contains "Yes" return TRUE 
                  ~str_detect(.x, "Yes"), 
                  ## rename with original name with "_simple" on the end
                  .names = "{.col}_simple")
    )

## create a list of vaccines with simplified variable names
CARDVARSSIMP <- str_glue("{CARDVARS}_simple")

## create a list of vaccines with date on the front of variable names
CARDVARSDATE <- str_glue("date_{CARDVARS}")

## years since vaccination variables -------------------------------------------

## adapt date of vaccination and then create a new time_since variable
study_data_cleaned <- study_data_cleaned %>% 
  mutate(
    across(
      all_of(CARDVARSDATE), 
...
## create a grouping variable for years since vaccination
study_data_cleaned <- study_data_cleaned %>% 
  mutate(
    across(
      all_of(str_glue("{CARDVARSDATE}_time")), 
      ~age_categories(.x,
               lower = 0, upper = 5, by = 1) %>% 
        ## remove what comes after a dash (i.e. repeated year)
        str_remove("-.*") %>% 
        factor(levels = c("0", "1", "2", "3", "4", "5+")),
      ## rename with original name with "_grp" on the end
      .names = "{.col}_grp"
      ) 
      )

## create a list of vaccines with time group on the of variable names
CARDVARSTIMEGRP <- str_glue("{CARDVARSDATE}_time_grp")
```
...
## add in variables for newly created variables (based on old names)
## e.g. here label the simple vacc variables with the original variable labels
## we use c() to repeat this for the time since vaccination variable too
vacc_var_labels <- setNames(
  ## add variable labels as a list 
  as.list(
    c(study_data_dict$short_name[study_data_dict$type == "vaccine_card"], 
          study_data_dict$short_name[study_data_dict$type == "vaccine_card"])), 
  ## name the list with the variable names of interest
  c(CARDVARSSIMP, CARDVARSTIMEGRP)
)
...
study_data_cleaned <- study_data_cleaned %>% 
  set_variable_labels(
    ## set the labels with the object defined above
    .labels = vacc_var_labels, 
    ## do not throw an error if some names dont match 
    ## not all names in the dictionary appear in the dataset 
    .strict = FALSE)

## Adding labels for variables with multiple responses are allowed 
  ## In Kobo - multioption variables apear once in the dictionary 
  ## but exports have a character variable with all responses and then a 
  ## binary (1/0) variable for each of the various options 
  ## These are not automatically labelled from the dictionary above 

## first create a list of the binary variable names by combining with the 
## dictionary defined options for that variable 

reason_vars <- study_data_dict_long %>% 
  filter(type == "reasons") %>% 
  select(option_name, option_label_english) %>% 
  mutate(option_name = str_glue("reason_not_all_vacc_{option_name}"))


## add in variables for newly created variables (based on old names)
reason_var_labels <- setNames(
  ## add variable labels as a list 
  as.list(reason_vars$option_label_english), 
  ## name the list with the variable names of interest
  reason_vars$option_name
)
...
card-reported vaccination status, and detailing availability of vaccination cards
...
coverage while accepting equal validity from self- and card-reporting, then 
listing wether cards and records are available. 
...
survey_design %>% 
  ...
  select(any_vaccine, vaccine_card, hf_records) %>% 
  ## only display one line for each variable (with counts for "Yes")
  tbl_svysummary(value = list(everything() ~ "Yes")) %>% 
...
## Vaccination history verbal

Weighted overall vaccination completeness; based on surveyor assessments of
care-giver verbal accounts and on care-giver's own assessment

<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// vaccination_completeness_assessment \\\
--------------------------------------------------------------------------------

The below chunk creates a weighted table of counts proportions for vaccination 
completeness care-giver verbal-reporting, based on surveyor's assessment and on 
care-giver's own assessement of the verbal reporting of vaccinations
...
```{r vaccination_completeness_assessment}

survey_design %>% 
  ## only keep variables of interest
  select(all_vaccines_survey_woc, all_vaccines_caretaker_woc) %>% 
  ## only display one line for each variable (with counts for "Yes")
  tbl_svysummary(value = list(everything() ~ "Yes")) %>% 
  ## add the weighted total
  add_n() %>%
...
Weighted vaccination coverage by disease based on verbal history 

<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// vaccination_coverage_verbal \\\
--------------------------------------------------------------------------------

The below chunk creates a weighted table of counts proportions for vaccination 
coverage by each disease based on verbal history, it combines binary and 
categorical variables in to one table. 
...
```{r vaccination_coverage_verbal}

survey_design %>% 
  ## only keep variables of interest
  select(injection_upper_arm,
         scar_present,
         poliodrop_woc,
         poliodrop_hf_woc,
         poliodrop_camp_woc,
         ...
         num_rotavirus_woc) %>% 
  ## only display one line for each variable (with counts for "Yes")
  ## also specify for the scar_present variable to show the relevant level
  tbl_svysummary(value = list(where(is.factor) ~ "Yes", 
                 scar_present ~ "Scar present")) %>% 
  ## add the weighted total
  add_n() %>%
  ## add in confidence intervals
...
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// vaccination_completeness_assessment_card \\\
--------------------------------------------------------------------------------

The below chunk creates a weighted table of counts proportions for vaccination 
completeness based on surveyor's assessment of vaccination cards
...
```{r vaccination_completeness_assessment_card}

survey_design %>% 
  ## only keep variables of interest
  select(all_vaccines_survey_card) %>% 
  ## only display one line for each variable (with counts for "Yes")
  tbl_svysummary(value = list(everything() ~ "Yes")) %>% 
  ## add the weighted total
  add_n() %>%
...
Weighted vaccination coverage by vaccine and dose based on vaccination cards

<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// vaccination_coverage_card \\\
--------------------------------------------------------------------------------

The below chunk creates a weighted table of counts and proportions for vaccination 
coverage by vaccine and dose based on vaccination cards. Note that this uses a 
character vector of variable names in the dplyr::select() function, to pull the
...
```{r vaccination_coverage_card}

survey_design %>% 
  ## only keep variables of interest
  select(all_of(CARDVARSSIMP)) %>% 
  ## automatically only displays one line for each variable (with counts for TRUE)
  tbl_svysummary() %>% 
  ## add the weighted total
  add_n() %>%
  ## add in confidence intervals
...
Weighted counts of time (in years) since vaccination by various vaccines
based on vaccination cards

<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// vaccination_time_since \\\
--------------------------------------------------------------------------------

The below chunk creates a weighted table of counts and proportions time in years
since vaccination, by various vaccines based on assessment of cards. 
...
```{r vaccination_time_since}


survey_design %>% 
  ## only keep variables of interest
  select(all_of(CARDVARSTIMEGRP)) %>% 
  ## automatically only displays one line for each variable (with counts for TRUE)
  tbl_svysummary() %>% 
  ## add the weighted total
  add_n() %>%
...
Weighted counts and proportions for reasons not vaccinated, individually, among those not vaccinated based on verbal or card assessment
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// no_vacc_reason \\\
--------------------------------------------------------------------------------
...
survey_design %>% 
  ...
  filter(
    all_vaccines_survey_card != "Yes") %>%
  select(reason_vars$option_name) %>% 
  tbl_svysummary(value = list(everything() ~ 1)) %>% 
  ## add in confidence intervals
