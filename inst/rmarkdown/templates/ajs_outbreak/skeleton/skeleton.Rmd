...
bind_rows(
  lapply(
    SYMPTOMS, function(i) {
      # run descriptive function to get counts
      descriptive(linelist_cleaned, i, coltotals = TRUE) %>%
        # shift to be wide (note the need for !!sym as opposed to SEval for descriptive)
        gather(key = "variable", value = "value", c(.data$n, .data$prop)) %>%
        unite("temp", !!sym(i), .data$variable, sep = "_") %>%
        spread(.data$temp, .data$value)
    }
  )
) %>%
  # add var for symptom names after binding rows
  mutate(Symptom = SYMPTOMS) %>%
