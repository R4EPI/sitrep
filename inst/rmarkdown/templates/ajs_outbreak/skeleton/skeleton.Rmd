...
pacman::p_load(
  ...
  tidylog,         # to print effects of data cleaning
  patchwork,       # combine multiple plots
  sitrep,          # MSF field epi functions
  epitabulate,     # nice tables
  janitor,         # clean data
...
reporting_date <- ymd("2019-06-17")

## This command sets the reporting week (e.g 2019 W25) based on the reporting_date
...
linelist_raw <-  sitrep::gen_data("AJS")
...
# linelist_raw <- rio::import(here::here("linelist.xlsx"), which = "Sheet1")



## Excel file with password ----------------------------------------------------
## Use this section if your Excel has a password
## Change "linelist.xlsx" to the name and filepath of your file

# pacman::p_load(c("excel.link", "askpass"))

# linelist_raw <- xl.read.file(here::here("linelist.xlsx"),
#                              xl.sheet = "Sheet1",
...
## Excel file ------------------------------------------------------------------
## to import  a specific sheet use the "which" argument
# linelist_raw <- rio::import(here::here("linelist.xlsx"), which = "Sheet1")



## Excel file -- Specific range ------------------------------------------------
## this command allows you to specify a range in an excel sheet.
# linelist_raw <- rio::import(here::here("linelist.xlsx"), range = "B2:J102")



## Excel file with password ----------------------------------------------------
## use this section if your Excel has a password. The final command will prompt 
## you to enter the password.

## This command installs and loads packages that handle passwords
# pacman::p_load(excel.link, askpass)

## THis commands imports your password-protected Excel and asks for the password
# linelist_raw <- xl.read.file(here::here("linelist.xlsx"),
#                              xl.sheet = "Sheet1",
#                              password = askpass::askpass(prompt = "please enter file password"))
...
## CSV file --------------------------------------------------------------------
# linelist_raw <- rio::import(here::here("linelist.csv"))



## Stata data file -------------------------------------------------------------
# linelist_raw <- rio::import(here::here("linelist.dat"))
```
...
## AJS dictionary. This will copy a rename() command to your clipboard.

# msf_dict_rename_helper("AJS")

## Paste the result in the place indicated below.  
## Then, in the spaces indicated, write your data's column names that correspond
## to the standard column names in the MSF dictionary.  
...
## example, we have the columns "gender" and "age" in our dataset, which we want
## to map to (change to) the standard MSF column names "sex" and "age_years". 
...
#   rename(                # rename columns (syntax is NEW NAME = OLD NAME)
#     age_group = groups,
#     population = n)
...
# lab_data <- rio::import(here::here("labresults.xlsx"), which = "Sheet1")
...
## three columns, and age_years, and sex.
...
## remove exclude the names of variables which are completely empty
...
## Place the criteria on the left, and the correct date on the right of the ~
...
## See Epi R Handbook strings chapter for more details
## Use negate=TRUE argument to reverse the search
linelist_cleaned <- linelist_cleaned %>% 
...
# before 4-24, etc
linelist_cleaned <- linelist_cleaned %>% 
  mutate(time_to_death = fct_relevel(time_to_death,
           "0-4 hours", ">4-24 hours", ">24-48 hours", ">48 hours"))
...
cases <- linelist_cleaned %>% 
  count(age_group) %>%    # cases for each age_group
...
## define begining and end of period of interest 
start_week <- min(linelist_cleaned$epiweek, na.rm = TRUE)
end_week <- max(linelist_cleaned$epiweek, na.rm = TRUE)

## pull the unique weeks which occur
all_weeks <- seq(start_week,
                 end_week, by = 1) 
...
The peak of the outbreak was in `r tab_linelist(linelist_cleaned, epiweek) %>% slice(which.max(n)) %>% pull(value)`  
...
# This chunk will calculate weekly incidence and with a moving average.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
```{r epicurve_moving, message = FALSE}
# adjust the width of the moving average window here
moving_window_days <- 14

# calculates the moving average using counts by date of onset
moving_avg <- linelist_cleaned %>% 
  ...
  mutate(indexed_window = slider::slide_index_dbl(
        ...
        .i = date_of_onset,       # indexed with date_onset 
        .f = ~sum(.x, na.rm = TRUE),     # function is sum() with missing values removed
        .before = days(moving_window_days - 1)))

# create plot
moving_plot <- ggplot(data = moving_avg,
       mapping = aes(x = date_of_onset, y = indexed_window))+
  geom_line()+
...
```{r monthly_epicurve, message = FALSE}
## Create epimonth variable -----------------------------------------------------
## This step creates an epimonth variable from the date of onset.
## You can use date_of_consultation_admission if you are missing many date_of_onset.
linelist_cleaned <- linelist_cleaned %>% 
  mutate(
    ## create an epiweek variable 
    epimonth = tsibble::yearmonth(
      date_of_onset),
    ## create a date version of epiweek 
    epimonth_date = as.Date(epiweek)) 


## define begining and end of period of interest 
start_month <- min(linelist_cleaned$epimonth, na.rm = TRUE)
end_month <- max(linelist_cleaned$epimonth, na.rm = TRUE)

## pull the unique weeks which occur
all_months <- seq(start_month,
                 end_month, by = 1) 

## change weeks to dates
all_months_date <-  as.Date(all_months)  


## define ggplot themes
tsa_axes <-
  ## set the x-axis limits based on weeks of interest, only label every january
  scale_x_date(breaks = all_months_date,
               labels = all_months,
               limits = c(
                 first(all_months_date),
                 last(all_months_date)
               ))


monthly_curve <- linelist_cleaned %>%
  filter(!is.na(epimonth_date)) %>%
  
  ## plot those not missing using the date variable 
  ggplot(aes(x = epimonth_date)) +
...
sex_curve <- linelist_cleaned %>%
  ...
  # control the fill color of the bars
...
case_def_curve <- linelist_cleaned %>%
  filter(!is.na(epiweek_date)) %>%
...
facility_curve_sex <- linelist_cleaned %>%
  filter(!is.na(epiweek_date)) %>%
  filter(patient_facility_type == "Inpatient") %>% 
...
cfr <- linelist_cleaned %>%
  filter(patient_facility_type == "Inpatient") %>%
  mutate(epiweek_char = as.character(epiweek)) %>% 
  case_fatality_rate_df(str_detect(exit_status, "Dead"), group = epiweek_char)
...
cfr_plot <- ggplot(cfr, aes(x = as.Date(yearweek(as.character(epiweek_char)))), group = 1) +
  # add confidence intervals as a ribbon
  geom_ribbon(aes(ymin = lower, ymax = upper), 
              color = "red", fill = "red", 
              linetype = 2, alpha = 0.2, show.legend = FALSE) +
...
linelist_cleaned <- linelist_cleaned %>% 
  mutate(patient_origin <- as_factor(patient_origin))
...
for (week in unique(cases$epiweek)) {

  week <- yearweek(as.Date(week, origin = "1970-01-01"))
  this_ar <- ar %>% filter(epiweek == week)
