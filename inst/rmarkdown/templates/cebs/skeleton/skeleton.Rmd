...
If you didn't use KoBo data, go to read_Excel_data.
...
```{r read_Excel_data, warning = FALSE, message = FALSE}
...
```{r prep_Excel_data, warning = FALSE, message = FALSE}
## MSF CEBS Dictionary ----------------------------------------------------------
## get MSF standard dictionary for CEBS 
...
# linelist_reduced <- select(cebs_cleaned, c(1:3, "age_years", "sex"))
...
## here is an example:
# table(cebs_cleaned$date_of_consultation_admission)
...
cebs_cleaned$epiweek <- aweek::date2week(cebs_cleaned$date_of_onset, 
                                             floor_day = TRUE, 
                                             factor = TRUE)
...
cebs_cleaned$age_group_mon <- age_categories(cebs_cleaned$age_months, 
                                                 breakers = c(0, 6, 9, 12, 23), 
                                                 ceiling = TRUE)

## create an age group variable by specifying categorical breaks
cebs_cleaned$age_group <- age_categories(cebs_cleaned$age_years, 
...
# cebs_cleaned$age_group <- age_categories(cebs_cleaned$age,
#                                              lower = 0, 
#                                              upper = 100, 
#                                              by = 10)
...
# cebs_cleaned$age_group <- factor(cebs_cleaned$age_group,
...
##
# cebs_cleaned <- group_age_categories(cebs_cleaned, 
#                                          years = age_group, 
