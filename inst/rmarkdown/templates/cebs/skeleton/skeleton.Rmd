...
ewar_cleaned %>% 
  ## Filter to only include data for the past 4 weeks
...
Across the period of evaluation, there were `r nrow(ewar_cleaned, ALERT)` alerts of which `r nrow(filter(ewar_cleaned, ALERT, response_undertaken == "y"))` resulted in public health action.
...
ewar_cleaned %>% 
  ...
  select(detection_time, verification_time, assessment_time, response_time) %>% 
  ...
  tbl_summary(missing = "no",
              ...
              label = list(detection_time ~ "Time to detect signal" ,
                ...
                assessment_time ~ "Time to assess verified signal",
                response_time ~ "Time from assessment to response")) %>% 
  ## convert to flextable
  gtsummary::as_flex_table()
