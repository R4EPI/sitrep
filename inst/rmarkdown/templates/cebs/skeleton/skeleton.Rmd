...
## set 6 weeks prior to reporting week
six_weeks_ago <- reporting_week - 6
...
first_week <- aweek::as.aweek(min(as.character(ewar_cleaned$epiweek)))
...
#### Time
##### Overview of EBS performance indicators
<!-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
```{r time_overview_table}

summary_table <- ewar_cleaned %>% 
  ## Filter to only include data for the past 6 weeks
  filter(epiweek >= six_weeks_ago & epiweek <= reporting_week) %>% 
  ## Group by epiweek
  group_by(epiweek) %>% 
...
# Transpose the data so that epiweeks are on top
summary_table_t <- data.table::transpose(summary_table, keep.names = "rn") %>% 
   as_tibble() 

# Change the column names to take on the first row values
colnames(summary_table_t) = summary_table_t[1,]

## Finalise table
summary_table_t %>% 
  ## remove the 2nd row of data
  slice(-1) %>% 
  ## make a flex table
  flextable()
...
##### Verified signals by type over time
```{r time_verified_signals_type_table}

verif_sig_table <- ewar_cleaned %>% 
  ## Filter to only include data for the past 6 weeks and verified signals
  filter(epiweek >= six_weeks_ago & epiweek <= reporting_week & event_status == "1") %>% 
  ## Group by epiweek
  group_by(epiweek) %>% 
...
# Transpose the data so that epiweeks are on top
verif_sig_table_t <- data.table::transpose(verif_sig_table, keep.names = "rn") %>% 
   as_tibble() 

# Change the column names to take on the first row values
colnames(verif_sig_table_t) = verif_sig_table_t[1,]
...
```{r time_alerts_type_figure}

alert_table <- ewar_cleaned %>% 
  ## Filter to only include data for the past 6 weeks and alerts
  filter(epiweek >= six_weeks_ago & epiweek <= reporting_week & ALERT == TRUE) %>% 
  ## Group by epiweek
  group_by(epiweek) %>% 
  ## Count the number of verified signals by signal type
  ## You will need to replace the new variable names below to reflect the 
...
# Transpose the data so that epiweeks are on top
alert_table_t <- data.table::transpose(alert_table, keep.names = "rn") %>% 
   as_tibble() 

# Change the column names to take on the first row values
colnames(alert_table_t) = alert_table_t[1,]
...
##### Alerts by type by location - table
<!-- The below shows all signals in the past 6 weeks. The time period can be adjusted  -->
<!-- as needed -->
```{r place_alerts_type_table}
...
```{r place_verified_signals_type_map}
...
During the evaluation period, there were `r fmt_count(filter(ebs_cleaned, epiweek >= start_week & epiweek <= reporting_week), is.na(date_signal)== F)`, `r fmt_count(filter(ebs_cleaned, epiweek >= start_week & epiweek <= reporting_week), is.na(event_id)== F)`, `r fmt_count(filter(ebs_cleaned, epiweek >= start_week & epiweek <= reporting_week), ALERT == TRUE)`,  `r fmt_count(filter(ebs_cleaned, epiweek >= start_week & epiweek <= reporting_week), response_undertaken == "y")` signals collected, events identified, alerts reported and responses conducted, respectively.
