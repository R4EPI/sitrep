...
pacman::p_load(
  ...
  purrr,         # iteration
  naniar,        # Missing data
  httr,          # for using KoBo API 
  jsonlite,      # for using KoBo API
  keyring        # for inputting pasword for KoBo
)
...
## set current week 
reporting_week <- tsibble::yearweek("2024-W15")
...
# ebs_raw <- left_join(signal_collect_raw, signal_verfication_raw, by = "signal_id") %>% 
              ## join the combined forms with the risk assessment form
              # left_join(risk_assement_raw, by = "signal_id") %>% 
