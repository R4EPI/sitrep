...
```{r map_for_loop_epiweek, fig.width = 12}
...
ar <- attack_rate(cases$n, cases$population, multiplier = 100000) %>% 
  ...
  rename("Region" = patient_origin_free_text, 
         ...
         "Lower 95%CI" = lower,
         "Upper 95%CI" = upper)
max_cases <- max(cases$n) # define the maximum number of cases for the color palette 

# go through each epiweek, fiter and plot the data
for (i in unique(linelist_cleaned$epiweek)) {
