...
count(linelist_cleaned, age_group, sex) %>%  # get counts of cases by age group and sex
  mutate(prop = n/nrow(linelist_cleaned)*100) %>%  # calculate the proportion in each based on total cases
  tidyr::gather(key = "variable", value = "value", c(.data$n, .data$prop)) %>% # collect n and props for each
  tidyr::unite("temp", .data$sex, .data$variable, sep = "_") %>% # group these with the sex var 
  tidyr::spread(.data$temp, .data$value) %>% # spread with age_group as your key
  rename("Age group (years)" = age_group, 
         "Female cases (n)" = Female_n, 
         "%" = Female_prop, 
         "Male cases (n)" = Male_n, 
...
# bind sex specific cfr and overall cfr together as rows
bind_rows(
  ...
  # add a column to overall_cfr with "Total" in it so can add as a row below
  overall_cfr %>% 
    mutate(Sex = "Total")
  ) %>% 
...
# bind age specific cfr and overall cfr together as rows
bind_rows(
...
# bind case def specific cfr and overall cfr together as rows
bind_rows(
...
cases <- count(linelist_cleaned, epiweek) %>% # get counts
  full_join(as.data.frame(inc_week_7), by = c(epiweek = "isoweeks", n = "counts")) %>%  # ensure all weeks there
  arrange(dates) %>% # sort by date
  mutate(cumulative = cumsum(n)) %>% # add the cummulative count
  mutate(epiweek = forcats::fct_inorder(epiweek)) # sort based on epiweek
...
cfr <- linelist_cleaned %>% 
  filter(!is.na(exit_status)) %>%               # remove rows with missing outcome
  group_by(epiweek) %>%                        # group by epiweek
  summarise(deaths = sum(grepl("Dead", exit_status)),  # tally deaths
            population = n()) %>%               # tally population
...
# ggplot() +
#   geom_sf(data = mapsub, aes(col = `AR (per 100,000)`)) + # shapefile as polygon
#   coord_sf(datum = NA) + # needed to avoid gridlines being drawn
#   annotation_scale() + # add a scalebar
#   scale_colour_viridis_c(option = "C") + # color the scale to be perceptually uniform
#   theme_void() # remove coordinates and axes
...
## - [ ] decide if you would like to show counts or AR
## - [ ] replace `Cases (n)` and `AR (per 100,000)` appropriately
## - [ ] consider facet wrapping by an overarching unit if have many regions (e.g. by province)
