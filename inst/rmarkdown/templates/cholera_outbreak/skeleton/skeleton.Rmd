...
required_packages <- c("knitr",       # create output docs
                       ...
                       "linelist",    # to clean variable codes to human-readable values
...
# the report. 
trtpln <- tab_linelist(linelist_cleaned, fluids_treatment_plan) %>%
  ...
  mutate(to_print = sprintf("%d (%.1f%%)", n, proportion))

# number of people in the plans A, B, and C
trtplnA <- filter(trtpln, grepl("A$", value)) %>% pull(to_print)
trtplnB <- filter(trtpln, grepl("B$", value)) %>% pull(to_print)
trtplnC <- filter(trtpln, grepl("C$", value)) %>% pull(to_print)
```
...
tab_linelist(linelist_cleaned, 
            ...
            col_total = TRUE) %>% 
  select(-variable) %>%
  rename("Dehydration severity" = value, 
         "Cases (n)" = n, 
         "%" = proportion) %>% 
  kable(digits = 2)
```
...
inc_week_7_sex_conf <- linelist_cleaned %>%
...
for (i in unique(cases$epiweek)) {
  ...
  map_plot <- ggplot() +
    geom_sf(data = mapsub, 
            aes(fill = categories), 
            col = "grey50") + # shapefile as polygon
    coord_sf(datum = NA) + # needed to avoid gridlines being drawn
    annotation_scale() + # add a scalebar
