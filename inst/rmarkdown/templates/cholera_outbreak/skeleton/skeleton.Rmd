...
## create a categories based on other categories 
linelist_cleaned <- linelist_cleaned %>% 
  mutate(patient_facility_type = if_else(
    cholera_treatment_facility_type == "Cholera Treatment Centre" | 
      cholera_treatment_facility_type == "Cholera Treatment Unit", 
    "Inpatient", 
    "Outpatient")
    )
...
There were `r fmt_count(linelist_cleaned, is.na(sex))` cases missing information on sex, 
`r fmt_count(linelist_cleaned, is.na(case_def))` missing case definitions and 
...
A total of `r fmt_count(linelist_cleaned, readmission == "Yes")` patients were readmitted to the CTC. 
...
CFR by case definition among inpatients with known outcomes
...
This section gives mortality rates attributable to cholera in a closed population. It does not calculate all-cause mortality. It assumes that all cholera deaths are among inpatients.
...
To produce a mortality rate (attributable to cholera) per 10,000 people use the following code chunk. 
This assumes that you are capturing every cholera death in your population. 
...
To produce a crude mortality rate attributable to cholera per 10,000 people per day, use the folowing 
code chunk. 
