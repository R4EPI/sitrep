...
rename(cfr, 
  ...
  knitr::kable(cfr, digits = 2)
```


You could plot the AR and CFR together as line graphs by epiweek. 

```{r collect_variables_lines}

ar_sub <- select(ar, epiweek, ar, lower, upper) %>% 
  mutate(group = "AR") %>% 
  rename("estimate" = ar)

cfr_sub <- select(cfr, epiweek, cfr, lower, upper) %>% 
  mutate(group = "CFR") %>% 
  rename("estimate" = cfr)

rates <- bind_rows(ar_sub, cfr_sub)

```



```{r line_graphs_ar_cfr}

lp <- ggplot(rates, aes(x = epiweek, group = group, fill = group, col = group)) + 
  geom_ribbon(aes(ymin = lower, ymax = upper), linetype = 2, alpha = 0.2,  show.legend = FALSE) +
  geom_line(aes(y = estimate)) + 
  scale_y_continuous(expand = c(0,0)) +  # set origin for axes
  # add labels to axes and below chart
  labs(x = "Calendar week", y = "Rate") + 
  # change visuals of dates and remove legend title
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), 
        legend.title = element_blank(), legend.position = "bottom") + 
  # change the labels of the legend
  scale_color_discrete(labels = c("AR[95%CI] (per 100,000)", "CFR[95%CI] (as %)"))
...
gridExtra::grid.arrange(basic_curve, lp, nrow = 2)
