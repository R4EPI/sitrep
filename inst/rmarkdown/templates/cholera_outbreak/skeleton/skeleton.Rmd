...
## get MSF standard dictionary for cholera 
...
linelist_cleaned <- linelist_cleaned %>%
  mutate(
    across(
      ...
      .cols = starts_with("dehydration"),  
      # Sets order of levels
      .fns = ~fct_relevel(.x, 
                          levels = c("Severe", "Some", "None", "Unknown"))))  %>% 
...
linelist_cleaned %>% 
  ## only keep variables of interest
  select(fluids_treatment_plan, case_def) %>% 
  ## make NAs show up as "Missing" (so they are included in counts)
  mutate(case_def = forcats::fct_explicit_na(case_def, "Missing")) %>%
  ## create a table with counts/percentages by column
  tbl_summary(by = case_def,
              percent = "column",
              label = list(fluids_treatment_plan = "Treatment plan")) %>% 
  ## add totals
  add_overall() %>% 
  ## make variable names bold 
  bold_labels() %>% 
...
linelist_cleaned %>% 
  ...
  mutate(dehydration_level_at_admission = forcats::fct_explicit_na(dehydration_level_at_admission, "Missing")) %>%
  tbl_summary(percent = "cell",
              label = list(dehydration_level_at_admission = "Dehydration severity")) %>% 
  ## make variable names bold 
  bold_labels() %>% 
  # change to flextable format
  as_flex_table() %>%
...
linelist_cleaned %>% 
  select(dehydration_severity_during_stay) %>% 
  mutate(dehydration_severity_during_stay = forcats::fct_explicit_na(dehydration_severity_during_stay, "Missing")) %>%
  tbl_summary(percent = "cell",
              label = list(dehydration_severity_during_stay = "Dehydration severity")) %>% 
  ## make variable names bold 
  bold_labels() %>% 
  # change to flextable format
  as_flex_table() %>%
...
linelist_cleaned %>% 
  ...
  tbl_summary(percent = "cell",
              label = list(previously_vaccinated = "Previous vaccination")) %>% 
  ## make variable names bold 
  bold_labels() %>% 
  # change to flextable format
  as_flex_table() %>%
...
To produce a mortality rate (attributable to cholera) per 10,000 people use
