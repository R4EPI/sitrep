...
# you can look at the standard dictionary with this
# View(linelist_dict) 
# you can view variable names in the standard data dicationry with this
# linelist_dict$data_element_shortname


# THIS LINE CAN BE DELETED - YOU READ IN YOUR OWN DATA ABOVE!
...
# UNCOMMENT AND RUN THREE LINES OF CODE BELOW!
...
# OPTIONAL: below is just an example in case you want to specifically change a few names)
...
# OPTIONAL: if you only want to keep certain variables - you can select these by name or column number
# view the names of your vars and their column number using: names(linelist_cleaned)
# this example keeps the first three columns as well as age_years and sex variables
# linelist_cleaned <- select(linelist_cleaned, c(1:3, "age_years", "sex")



# OPTIONAL: ONLY RUN THESE LINES IF YOUR DATA MATCHES THE DICTIONARY EXACTLY(!) AND YOU DONT WANT ANY EXTRA VARS
# find out which variables match the data dictionary 
match_cols <- match(colnames(linelist_cleaned), linelist_dict$data_element_shortname)

# only keep variables defined data dictionary 
linelist_cleaned <- select(linelist_cleaned, match_cols)
# alternatively keep the matched columns and specify names of extra columns in your data you want to keep
# linelist_cleaned <- select(linelist_cleaned, c(match_cols, "age_years", "sex"))

# switch variable contents to show what you see on data entry (i.e. named rather than coded values) 
# this is only for the variables matching between the two
linelist_cleaned[match_cols] <- switch_vals(df = linelist_cleaned[match_cols], disease = "Cholera")
```
...
# You will need to clean and update this script to match your data
  # This will also require you to recode variable contents
  # You will also need to update the code later in the script to fit your names/contents
# You will still need to read in the standardised data dictionary
  ...
  # you will need to run this to look at the data dictionary: 
  # linelist_dict <- msf_dict("Cholera")
  # View(linelist_dict) 
