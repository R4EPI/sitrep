...
# before the equals is the old name and after is the new name
...
#                             readmittedvar   = readmission                     ,
#                             pregnantoutcome = pregnancy_outcome_at_exit       ,
...
linelist_cleaned <- linelist_cleaned %>% 
  mutate(dehydration_level_at_admission = factor(dehydration_level_at_admission, 
                                                 levels = c("None", 
                                                            "Some", 
                                                            "Severe", 
                                                            "Unknown")
                                                 ),
         dehydration_severity_during_stay = factor(dehydration_severity_during_stay,
                                                   levels = c("None", 
                                                            "Some", 
                                                            "Severe", 
                                                            "Unknown"))
...
From the start of the outbreak, `r paste0(filter(descriptive(linelist_cleaned, counter = "fluids_treatment_plan"), fluids_treatment_plan == "Treatment plan A") %>% select(n), " (", filter(descriptive(linelist_cleaned, counter = "fluids_treatment_plan"), fluids_treatment_plan == "Treatment plan A") %>% select(prop), "%)")` 
patients were admitted to treatment plan A, 
`r paste0(filter(descriptive(linelist_cleaned, counter = "fluids_treatment_plan"), fluids_treatment_plan == "Treatment plan B") %>% select(n), " (", filter(descriptive(linelist_cleaned, counter = "fluids_treatment_plan"), fluids_treatment_plan == "Treatment plan B") %>% select(prop), "%)")`
were admitted to treatment plan B, with 
`r paste0(filter(descriptive(linelist_cleaned, counter = "fluids_treatment_plan"), fluids_treatment_plan == "Treatment plan C") %>% select(n), " (", filter(descriptive(linelist_cleaned, counter = "fluids_treatment_plan"), fluids_treatment_plan == "Treatment plan C") %>% select(prop), "%)")`
being admitted to treatment plan C. 
...
A total of `r filter(linelist_cleaned, readmission == "Yes") %>% count()` patients were readmitted to the CTC. 
...
To date, there have been `r sum(grepl("Dead", exit_status))` deaths, of which 
`r linelist_cleaned %>% filter(exit_status == "Dead on arrival") %>% count()` 
...
linelist_cleaned %>% 
  ...
  summarise(deaths = sum(exit_status == "Dead in facility" |
                           exit_status == "Dead on arrival"),        # tally deaths
...
mortality_rate(deaths, pat_obs_time$days, multiplier = 10000) %>%
  rename("Deaths" = deaths, 
         "Population" = population, 
         "Mortality (per 10,000/day)" = `mortality per 10 000`, 
         "Lower 95%CI" = lower,
...
The below gives case fatility rate per week. 
...
kable(ar, digits = 2, format.args = list(big.mark = ",")) # set thousands separator
```
