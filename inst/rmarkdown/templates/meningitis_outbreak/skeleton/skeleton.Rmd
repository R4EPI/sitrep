...
The packages used in this template are:

- *knitr*: for creating output documents
- *dplyr*: for cleaning/shaping data
- *forcats*: for cleaning/shaping data
- *ggplot2*: for plotting charts
- *sitrep*: for MSF field epi functions
- *incidence*: for epicurves
- *aweek*: for creating epiweeks
- *epitools*: for 2x2 tables and other epi goodness
- *sf*: for encoding spatial vector data
- *ggspatial*: for plotting maps



```{r, echo = T, eval=FALSE, results='hide', message=FALSE, warning=FALSE}
# Installing required packages for this case study
required_packages <- c("knitr", "dplyr", "forcats", "ggplot2",
                       "sitrep", "incidence", "aweek", "epitools",
                       "sf", "ggspatial") 
...
linelist_raw <- gen_data("Meningitis") # generates a fake dataset for use as an example in this template
...
## get MSF data dictionary for meningitis 
linelist_dict <- msf_dict("Meningitis")
...
linelist_cleaned <- switch_vals(df = linelist_cleaned, disease = "Meningitis")
```
...
## on the meningitis dictionary. 
## This will copy a rename command like the one above ## to your clipboard.
...
```{r lab_data}
## OPTIONAL: this section can be used to read in separate lab data
## make sure that your unique identifiers (e.g. case_number) match
## with the linelist! 
##
## Generally, lab data is included with the meningitis line list.
## If you have additional lab data, you can use this section.
##
...
## Fake data ------------------------------------ 
## THE GENERATE SECTION CAN BE DELETED IF YOU HAVE YOUR 
## OWN (REAL) LABORATORY DATA! 
## it's just to be able to demonstrate script in this template
# 
...
# create a new grouping for exit status variable & combine vaccination vars
linelist_cleaned <- linelist_cleaned %>% 
...
# definition variable. Please adapt to your outbreak case definition.
...
linelist_cleaned <- linelist_cleaned %>%
  mutate(date_of_last_vaccination = ifelse(
    ...
    as.Date(date_of_last_vaccination), as.Date(NA)
  ))
...
Alternatively if you would like proportions to be of the total linelist,
use the following. 
...
Of the patients, `r fmt_count(linelist_cleaned, patient_facility_type == "Outpatient")` were seen as outpatients and `r fmt_count(linelist_cleaned, patient_facility_type == "Inpatient")` were inpatients. Among inpatients, the median number of days admitted was `r median(linelist_cleaned$obs_days)`, 
...
You can describe the lab results by the detailed Pastorex result.

```{r describe_detailed_pastorex}
descriptive(linelist_cleaned, 
            counter = "pastorex_result", 
            coltotals = TRUE) %>% 
  rename("Pastorex result" = pastorex_result, 
         "Cases (n)" = n, 
         "%" = prop) %>% 
  kable(digits = 2)
```

If available, you can also describe the PCR results you have.

```{r describe_pcr}
descriptive(linelist_cleaned, 
            counter = "ti_result_pcr", 
            coltotals = TRUE) %>% 
  rename("PCR result" = ti_result_pcr, 
         "Cases (n)" = n, 
         "%" = prop) %>% 
  kable(digits = 2)
```
...
Of `r linelist_cleaned %>% filter(patient_facility_type == "Inpatient") %>% count()` inpatients, there have been `r fmt_count(linelist_cleaned, grepl("Dead", exit_status), patient_facility_type == "Inpatient")` deaths, of which 
...
This section gives mortality rates attributable to meningitis in
a closed population. It does not calculate all-cause mortality. It 'assumes 
that all meningitis deaths are among inpatients.
...
To produce a mortality rate (attribuatble to meningitis) per 10,000 people
use the following code chunk. 
This assumes that you are capturing every meningitis death in your population. 
...
To produce a crude mortality rate attributable to meningitis per 10,000 people
per day, use the folowing code chunk. 
This assumes that you are capturing every meningitis death in your population 
and that your population remains stable over the time period of interest. 
...
You could also plot biweekly if needed. There are also the options to do this
...
inc_week_7_vax <- incidence(linelist_cleaned$date_of_onset, 
                        interval = "Monday week", 
                        groups = linelist_cleaned$previously_vaccinated)
...
inc_week_7_sex_fac <- incidence(linelist_cleaned$date_of_onset[linelist_cleaned$patient_facility_type == "Inpatient"], 
                        interval = "Monday week", 
                        groups = linelist_cleaned$sex[linelist_cleaned$patient_facility_type == "Inpatient"])
...
ar <- attack_rate(cases$n, population, multiplier = 100000) %>% 
  ...
  tidyr::complete(epiweek, 
                  fill = list(cases = 0, population = 0, ar = 0, lower = 0, upper = 0)) %>% # Ensure all levels are represented
...
cfr <- linelist_cleaned %>%
  ...
  tidyr::complete(epiweek, fill = list(deaths = 0, population = 0, cfr = 0, lower = 0, upper = 0)) %>% # Ensure all levels are represented
  mutate(epiweek = as.character(epiweek))
...
cfr_plot <- ggplot(cfr, aes(x = epiweek, group = 1)) +
  ...
  # add labels to axes and below chart
  labs(x = "Calendar week", y = "CFR [95% CI]", subtitle = "Case fatality ratio [95% CI] among inpatients") + 
  # change visuals of dates and remove legend title
  epicurve_theme 
```
...
descriptive(linelist_cleaned, "epiweek", "patient_facility_type", coltotals = TRUE, rowtotals = TRUE) %>% 
  rename("Week" = epiweek, 
         "Inpatient cases (n)" = Inpatient_n, 
         "%" = Inpatient_prop, 
         "Outpatient cases (n)" = Outpatient_n, 
         "%" = Outpatient_prop) %>% 
  kable(digits = 2)
```
...
descriptive("epiweek", "exit_status2", 
  ...
  rename("Week" = "epiweek", 
         "Died (n)" = "Died_n", 
         "%" = "Died_prop", 
         "Discharged (n)" = "Discharged_n", 
         "%" = "Discharged_prop", 
         "Left (n)" = "Left_n", 
         "%" = "Left_prop", 
         "Transferred (n)" = "Transferred_n", 
         "%" = "Transferred_prop", 
         "Total" = "Total"
         ) %>% 
  kable(digits = 2)
...
descriptive(linelist_cleaned, "patient_origin", 
  ...
  rename("Region" = "patient_origin", 
         "Inpatient (n)" = "Inpatient_n", 
         "%" = "Inpatient_prop", 
         "Outpatient (n)" = "Outpatient_n", 
         "%" = "Outpatient_prop",
         "Total" = "Total"
         ) %>% 
  kable(digits = 2)
