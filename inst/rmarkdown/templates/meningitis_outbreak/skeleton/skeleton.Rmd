...
# change to dates 
linelist_cleaned <- linelist_cleaned %>%
  mutate_at(DATEVARS, linelist::guess_dates, error_tolerance = 0.5)
...
The most affected age group was `r tab_linelist(linelist_cleaned, age_group) %>% slice(which.max(n)) %>% pull(age_group)` years. 
...
tab_linelist(linelist_cleaned, pastorex_result,
             col_total = TRUE) %>%
  select(-variable) %>%
