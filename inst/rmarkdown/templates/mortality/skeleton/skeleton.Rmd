...
study_data_cleaned <- study_data_cleaned %>% 
  mutate(q45_q29_hh_leave_date = ifelse(q45_q29_hh_leave_date > recall_end, as.Date(NA), 
...
# WE WANT TO IGNORE THOSE WHO HAVE A MISSING ID (i.e. drop empty excel rows)
# WE WANT TO IGNORE THOSE THAT ARRIVED AFTER RECALL PERIOD 
# OR LEFT BEFORE RECALL PERIOD (WITHOUT AN ARRIVAL IN RECALL - because ppl might come and go)
study_data_cleaned <- study_data_cleaned %>%
  filter(!is.na(sex) &
