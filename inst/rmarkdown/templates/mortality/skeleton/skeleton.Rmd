...
# --- exchange with add_weight! ---
# keep for now (as add_weights does not seem to work properly)
...
study_data_cleaned <- study_data_cleaned %>% 
  mutate(weight_ID = paste0(age_group, "_", sex))
study_data_cleaned <- left_join(study_data_cleaned, 	
                                population_data_age, 	
                                by = c("weight_ID" = "ID"))
# --- exchange with add_weight! --- (end)
...
cause_of_death %>% 
  mutate(prop = fmt_pci_df(., which(colnames(.) == "proportion"), percent = FALSE, digits = 1),
         mort = fmt_ci_df(., which(colnames(.) == "mortality"), percent = FALSE, digits = 2)) %>% 
  select(cause_of_death, n, prop, mort) %>% 
  rename("Cause of death" = cause_of_death, 
         "% (95%CI)" = prop,
         "Mortality per 10,000 persons/day (95%CI)" = mort) %>% 
  kable()
...
cause_of_death_age <- survey_design %>% 
...
cause_of_death_age %>% 
  mutate(prop = fmt_pci_df(., which(colnames(.) == "proportion"), percent = FALSE, digits = 1)) %>% 
  select(cause_of_death, age_group, n, prop) %>% 
  gather(variable, value, -(cause_of_death:age_group)) %>%
  unite(temp, age_group, variable, sep = " ") %>% 
  spread(temp, value) %>% 
  rename_all(. %>% gsub("prop", "% (95%CI)", .)) %>% 
  kable()
...
cause_of_death_sex %>% 
  mutate(prop = fmt_pci_df(., which(colnames(.) == "proportion"), percent = FALSE, digits = 1)) %>% 
  select(cause_of_death, sex, n, prop) %>% 
  gather(variable, value, -(cause_of_death:sex)) %>%
