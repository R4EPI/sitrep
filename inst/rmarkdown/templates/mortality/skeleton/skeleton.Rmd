...
net_change <- round( (1 - abs(at_start - at_end) / at_start) * 100, digits = 1)[[1]]
```
...
```{r visualise_population_flow, warning = FALSE}

# summarize data
flow_table <- study_data_cleaned %>%
  group_by(startcause, endcause, sex) %>%
  count()

# collect parts needed for plotting 
# grouping only by start and end cause, sex seperate
flow_table <- gather_set_data(flow_table, 1:2)

# make sure factor levels are correct 
# SEEMS TO IGNORE FACTOR LEVELS AND PLOT ALPHABETICALLY!
flow_table <- flow_table %>%
  ungroup() %>%  # not sure this ungroup is necessary!?
  mutate(
    x = factor(x, levels = c("startcause", "endcause")),
    startcause = factor(startcause, levels = c("Present at start", 
                                               "Born",
                                               "Other arrival")), 
    endcause = factor(endcause, levels = c("Present at end",
                                           "Died",
                                           "Other departure"))
  )
...
ggplot(flow_table, aes(x, id = id, split = y, value = n)) +
  # colour lines by sex 
  geom_parallel_sets(aes(fill = sex), alpha = 0.5, axis.width = 0.13) +
...
female ratio was `r sex_ratio_age %>% slice(which.min(ratio)) %>% pull(ratio)`
in the `r sex_ratio_age %>% slice(which.min(ratio)) %>% pull(age_group)`year age
group.
...
proportions of `r death_props %>% filter(died == TRUE) %>% pull(prop)`. 
...
# note that survey ratio includes the observation times in calculation
# note the posibility of negative confidence interval when low counts
cause_of_death_sex <- survey_design_sex %>% 
  group_by(sex) %>% 
  summarize(mortality = survey_ratio(as.numeric(died) * 10000, obstime, vartype = "ci")) %>% 
  # merge confidence intervals for mortality in to one column (do not change to percent)
  unite_ci("mortality", starts_with("mortality"), m100 = FALSE, percent = FALSE)
...
The gender specific mortality rate was `r cause_of_death_sex %>% slice(sex == "Female") %>% pull(mortality) ` deaths/10,000 persons/day amongst females and
`r cause_of_death_sex %>% slice(sex == "Male") %>% pull(mortality) `deaths/10,000 persons/day amongst males.
...
survey_design %>%                  # use the survey object (weighted)
  ...
  mutate(variable = SYMPTOMS_LABELS) %>% # correct symptom names in output table 
  select(-value) %>%                     # drop column which tells you which row values were kept 
...
```{r weighted_symptoms_reported_age, eval = FALSE}

### DOESNT WORK NEED TO ADD STRATIFIED TO TABULATEBINARYSURVEY
### issue is that when do pretty then %CI columns are same for strata and get ignored.... 

survey_design %>%                  # use the survey object (weighted)
  filter(q18_q8_sick == "Yes") %>% # filter for those who were sick in last 2 weeks
  # calculate weighted counts and weighted proportions - stratified by agegroup!!
  tabulate_binary_survey(SYMPTOMS, strata = age_group, keep = TRUE, pretty = TRUE) %>%
  mutate(variable = SYMPTOMS_LABELS) %>% # correct symptom names in output table 
  select(-value) %>%                     # drop column which tells you which row values were kept 
  rename("Symptom" = variable) %>%
  kable()
...
```{r weighted_symptoms_reported_sex, eval = FALSE}

### DOESNT WORK NEED TO ADD STRATIFIED TO TABULATEBINARYSURVEY
### issue is that when do pretty then %CI columns are same for strata and get ignored.... 


survey_design_sex %>%                  # use the survey object (weighted)
  filter(q18_q8_sick == "Yes") %>% # filter for those who were sick in last 2 weeks
  # calculate weighted counts and weighted proportions - stratified by sex!!
  tabulate_binary_survey(SYMPTOMS, strata = sex, keep = TRUE) %>%
  mutate(variable = SYMPTOMS_LABELS) %>% # correct symptom names in output table 
  select(-value) %>%                     # drop column which tells you which row values were kept 
  rename("Symptom" = variable,
         "Female reported" = "Female n",
         "Male reported" = "Male n",
        ) %>%
  kable()
```
