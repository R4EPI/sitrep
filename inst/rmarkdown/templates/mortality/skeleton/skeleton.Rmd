...
TODO: show breakdown as with measles with months and years combined 
...
cause_of_death_prop <- survey_design %>% 
  ...
  summarize(n = survey_total(vartype = "se", na.rm = TRUE),         # return weighted counts
            proportion = survey_mean(vartype = "ci", na.rm = TRUE)  # return weighted proportions
            ) %>% 
  # round the weighted counts to have whole numbers
  mutate(n = round(n)) %>% 
  # drop the standard error column produced by survey_total above (no need)
  select(-n_se)
...
dplyr::left_join(cause_of_death_prop, cause_of_death_mort) %>% 
  # merge the confidence intervals for proportions in to one column (and change to percent)
