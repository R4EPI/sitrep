...
bind_cols(
  ...
  # add a fisher's exact test for each age category
  mutate(pvalue = unlist(
    lapply(a$age_group, function(i) {
      top <- filter(a, age_group == i) %>% select(n, n1)
      bottom <- filter(a, age_group != i) %>% select(n, n1)
      counts <- bind_rows(
        top,
        colSums(bottom)
      )
  ...
  # correct p-value for those under 0.001
