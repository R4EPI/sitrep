...
Currently, value labels cant be used for analysis so we will just recode using 
the matchmaker package. Variable labels can be used and so we will used the 
labelled package for that. 

~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->

```{r labelling_data}

## label variable names with labelled 
## recode values with matchmaker (because value labels cant be used for analysis). 
...
checking <- msf_dict_survey(disease = "Mortality", compact = FALSE)

matchmaker::match_df(study_data_cleaned, 
                      checking, 
                      from = "option_name", 
                      to = "option_label_french", 
                      by = "name", 
...
## define temporary functions (TODO make these proper wrappers)
confidence_intervals <- function(data, variable, by, ...) {
  
  ## calculate confidence intervals using survey::svyciprop 
...
design_effect <- function(data, variable, by, ...) {
  
  ## design effect using survey::svymean 
  deffs <- svymean(as.formula(paste0( "~" , variable)), 
          data, na.rm = TRUE, deff = TRUE)
  
  ## pull using deff and only keep the TRUE column 
  ## (need better workaround for other var types
  deffs <- deff(deffs)[[TRUE]]
...
death_props <- survey_design %>% 
  ...
  ## add in deff 
  add_stat(fns = everything() ~ design_effect) %>% 
...
death_props_strat <- tab_survey(survey_design, died, 
                                ...
                                keep = "TRUE") %>%
  mutate(
    across(.cols = ends_with(" n"), 
            .fns = ~format(round(., digits = 1), big.mark = ",")))



survey_design %>% 
  # select(died, health_district) %>% 
  tbl_svysummary(by = died, 
                 include = c(died, health_district)) %>% 
  ## add the weighted total
  add_n() %>%   
  ## add in confidence intervals
  add_stat(fns = everything() ~ confidence_intervals) %>% 
