...
survey_design %>% 
  # filter(died) %>% 
  tabulate_survey(cause_of_death, age_group, prop_label = NULL) %>%
  rename("Cause of Death" = "cause_of_death") %>%
  rename_redundant(contains = "prop", label = "% (95% CI)") %>% # relabel all columns containing "prop" to "% (95% CI)"
  kable()
...
survey_design %>% 
  # filter(died) %>% 
  tabulate_survey(cause_of_death, sex, prop_label = NULL) %>%
  rename("Cause of Death" = "cause_of_death") %>%
  rename_redundant(contains = "prop", label = "% (95% CI)") %>% # relabel all columns containing "prop" to "% (95% CI)"
  kable()
