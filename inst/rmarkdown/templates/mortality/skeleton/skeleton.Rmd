...
# study_data_cleaned <- study_data_cleaned %>% 
#   mutate_at(vars(contains("_symptoms_")), ~if_else(is.na(.), "", .))
# TODO: Don't need to change these to TRUE/FALSE, we just need to make sure to
# collect all the non-NA levels and then use that in tabulate_binary_survey.

# SYMPTOMS <- grep("symptom", names(study_data_cleaned), value = TRUE) # get varsnames with symptom in
# SYMPTOMS <- SYMPTOMS[SYMPTOMS != "q20_q10_symptom_other"] # drop the "other" symptoms free text var 
...
survey_design %>%                  # use the survey object (weighted)
  ...
  tabulate_binary_survey(q19_q9_symptoms, keep = NA, invert = TRUE, method = "xlogit") %>%
  # mutate(variable = SYMPTOMS_LABELS) %>% # correct symptom names in output table 
  rename("Symptom" = q19_q9_symptoms,
         "N reported" = n,
         "% (95% CI)" = ci
        ) %>%
  kable()
...
survey_design %>%                  # use the survey object (weighted)
  ...
  tabulate_survey(q19_q9_symptoms, strata = age_group, pretty = TRUE) %>%
  rename("Symptom" = q19_q9_symptoms) %>%
  augment_redundant(" n" = " (n)") %>% # wrap all "n" variables in braces (note space before n).
  rename_redundant("ci"   = "% (95% CI)") %>% # relabel all columns containing "prop" to "% (95% CI)"
  kable()
...
survey_design %>%                  # use the survey object (weighted)
  ...
  tabulate_survey(q19_q9_symptoms, strata = sex) %>%
  rename("Symptom" = q19_q9_symptoms) %>%
  augment_redundant(" n" = " (n)") %>% # wrap all "n" variables in braces (note space before n).
  rename_redundant("ci"   = "% (95% CI)") %>% # relabel all columns containing "prop" to "% (95% CI)"
  kable()
