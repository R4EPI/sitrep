...
# using the fake dictionary (DELETE THIS)
...
study_data_cleaned <- study_data_cleaned %>%
  find_start_date(contains("born_date"),
                  contains("date_arrival"),
                  contains("arrive_date"),
                  cutoff = as.Date("2018-01-01"),
                  datecol = "startdate",
                  datereason = "startcause" 
                 ) %>%
  find_end_date(contains("leave_date"),
                contains("died_date"),
                cutoff = as.Date("2018-05-01"),
                datecol = "enddate",
                datereason = "endcause" 
               )
...
# for arrivals an dirths before the recall period, set to NA 
study_data_cleaned <- study_data_cleaned %>% 
  mutate(q114_q16_date_arrival_camp = ifelse(q114_q16_date_arrival_camp < recall_start, NA_real_, 
                                             q114_q16_date_arrival_camp), 
         q114_q16_date_arrival_camp = ifelse(q41_q25_hh_arrive_date < recall_start, NA_real_, 
                                             q41_q25_hh_arrive_date), 
         q88_q33_born_date = ifelse(q88_q33_born_date < recall_start, NA_real_, 
                                             q88_q33_born_date)
         )

# for deaths and departures after the recall period, set to NA 
study_data_cleaned <- study_data_cleaned %>% 
  mutate(q45_q29_hh_leave_date = ifelse(q45_q29_hh_leave_date > recall_end, NA_real_, 
                                             q45_q29_hh_leave_date), 
         q137_q35_died_date = ifelse(q137_q35_died_date > recall_end, NA_real_, 
                                             q137_q35_died_date)
         )
...
study_data_cleaned <- study_data_cleaned %>%
  filter(!is.na(sex) &
           (is.na(q114_q16_date_arrival_camp) |
              q114_q16_date_arrival_camp < recall_end) &
           (is.na(q41_q25_hh_arrive_date) | 
              q41_q25_hh_arrive_date < recall_end) & 
           (is.na(q88_q33_born_date) | 
              q88_q33_born_date < recall_end) 
           )
...
# correct startdate to born date if not NA (cant have arrived before born) 
study_data_cleaned <- study_data_cleaned %>% 
  mutate(startdate = ifelse(!is.na(q88_q33_born_date), 
                                       q88_q33_born_date, 
                                       startdate))
...
study_data_cleaned$startcause <- NA

# row wise 
for (i in 1:nrow(study_data_cleaned)) {
  
  # all three of the dates of interest are empty
  # set start cause to be start of recall period  
  if (length(which.min(study_data_cleaned[i, c("q114_q16_date_arrival_camp",
                                               "q41_q25_hh_arrive_date",
                                               "q88_q33_born_date")])) == 0) {
          study_data_cleaned[i, "startcause"] <- "Present at start"
  }
  # otherwise extract the name which was the minimum  
  else {
    study_data_cleaned[i, "startcause"] <- names(study_data_cleaned[i, c("q114_q16_date_arrival_camp",
                                                                         "q41_q25_hh_arrive_date",
                                                                         "q88_q33_born_date")])[
            which.min(study_data_cleaned[i, c("q114_q16_date_arrival_camp",
                                               "q41_q25_hh_arrive_date",
                                               "q88_q33_born_date")])]
  }
  
  if (!is.na(study_data_cleaned[i, "q88_q33_born_date"])) {
    study_data_cleaned[i, "startcause"] <- "Born"
  }
}


# make sure there are the appropriate levels
...
# start with an empty var
study_data_cleaned$endcause <- NA

# row wise 
for (i in 1:nrow(study_data_cleaned)) {
  
  # all three of the dates of interest are empty
  # set endcause to be end of recall period  
  if (length(which.min(study_data_cleaned[i, c("q45_q29_hh_leave_date", 
                                               "q137_q35_died_date")])) == 0) {
          study_data_cleaned[i, "endcause"] <- "Present at end"
  }
  # otherwise extract the name which was the minimum  
  else {
    study_data_cleaned[i, "endcause"] <- names(study_data_cleaned[i, c("q45_q29_hh_leave_date", 
                                                                       "q137_q35_died_date")])[
            which.min(study_data_cleaned[i, c("q45_q29_hh_leave_date", 
                                               "q137_q35_died_date")])]
  }
}
