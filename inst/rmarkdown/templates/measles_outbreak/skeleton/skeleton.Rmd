...
`r fmt_count(linelist_cleaned, str_detect(previously_vaccinated, "Yes"))` reported
previous vaccination. `r fmt_count(linelist_cleaned, previously_vaccinated == "No")` were unvaccinated and `r fmt_count(linelist_cleaned, previously_vaccinated == "Unsure")` were unsure.
...
This chunk creates a table of the severity of illness of your cases at
admission.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ -->
...
linelist_cleaned %>% 
  ...
  mutate(severity_of_illness = forcats::fct_explicit_na(severity_of_illness, "Missing")) %>%
  tbl_summary(percent = "cell",
              label = list(severity_of_illness = "Illness severity")) %>% 
  ## make variable names bold 
  bold_labels() %>% 
  # change to flextable format
  as_flex_table() %>%
...
To produce a mortality rate (attributable to measles) per 10,000 people use
...
linelist_cleaned %>%
  ...
  scale_x_date(
    breaks = all_weeks_date,
    labels = all_weeks) + 
  # use pre-defined labels (update the title)
  epicurve_labels +
  labs(title = "Cases by week of onset among inpatients") + 
  # use pre-defined theme (design) settings
  epicurve_theme
...
linelist_cleaned %>%
  ...
  ## plot those not missing using the date variable 
  ggplot(aes(x = epiweek_date, fill = fct_rev(fct_explicit_na(previously_vaccinated)))) +
  ## plot as a bar
  geom_bar(width = 7,
           colour = "black") +
  # align plot with bottom of plot
