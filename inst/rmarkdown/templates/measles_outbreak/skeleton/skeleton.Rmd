...
linelist_raw <- gen_data("Measles") # generates a fake dataset for use as an example in this template
...
population_data_region <- distinct(linelist_cleaned, patient_origin)
...
population_data_age_months <- tibble(age_group_mon = c("0-5", "6-8", "9-11","12-24"))
...
# make sure all the weeks are represented
linelist_cleaned <- linelist_cleaned %>% 
  mutate(ewdate = sprintf("%s-1", epiweek)) %>% 
  mutate(ewdate = ISOweek2date(ewdate)) %>% 
  mutate(epiweek = factor(epiweek, ISOweek(seq(min(ewdate, na.rm = TRUE), max(ewdate, na.rm = TRUE), by = "week")))) %>%
  select(-ewdate)
...
linelist_cleaned <- linelist_cleaned %>% 
  filter(date_of_onset <= ISOweek2date(sprintf("%s-7", reporting_week)))
...
obs_start <- ISOweek2date(sprintf("%s-1", first_week))

# return last day of reporting week 
obs_end   <- ISOweek2date(sprintf("%s-7", reporting_week))
...
`r fmt_count(linelist_cleaned, previously_vaccinated == "Yes - vaccination card/ink" | previously_vaccinated == "Yes - verbal")` reported previous vaccination. `r fmt_count(linelist_cleaned, previously_vaccinated == "No")` were unvaccinated and `r fmt_count(linelist_cleaned, previously_vaccinated == "Unsure")` were unsure.
...
THIS WHOLE MORTALITY SECTION SHOULD BE DELETED UNLESS YOU ARE IN A CLOSED SETTING (e.g. refugee camp)T. 
The assumptions don't hold in an open/community setting.

This section gives mortality rates attributable to measles in a closed population. It does not calculate all-cause mortality. It assumes that all measles deaths are among inpatients.
...
To produce a mortality rate (attribuatble to measles) per 10,000 people use the following code chunk. 
...
To produce a crude mortality rate attributable to measles per 10,000 people per day, use the folowing 
code chunk. 
This assumes that you are capturing every measles death in your population and that your population remains stable over the time period of interest. 
...
Alternatively, if you are unsure whether your hospital deaths are representative of the wider population, use the following code chunk. 
...
The below gives case fatality rate among inpatients per week as a proportion.
...
You could plot the AR and CFR (among inpatients only) together as line graphs by epiweek. 
...
cfr_plot <- ggplot(cfr, aes(x = epiweek, group = 1)) +
  ...
  # add labels to axes and below chart
  labs(x = "Calendar week", y = "CFR [95% CI]", subtitle = "Case Fatality Rate [95% CI] Among Inpatients") + 
  # change visuals of dates and remove legend title
  epicurve_theme 
```
...
```{r describe_admissions_by_epiweekf}
# get counts and props of age groups by case definition 
# include column and row totals 
linelist_cleaned %>%
  filter(patient_facility_type == "Inpatient") %>%
...
```{r describe_exits_by_epiweek}
# get counts and props of admissions by epiweek 
# include column and row totals 

linelist_cleaned %>%
  filter(patient_facility_type == "Inpatient") %>%
  descriptive("epiweek", "exit_status", 
            coltotals = TRUE, rowtotals = TRUE) %>% 
  rename("Week" = "epiweek", 
...
#### Descriptive
To get a basic descriptive table of cases by region and facility, and region and outcome among inpatients, see below. 
