...
## categories are 0-5m, 6-11m, 12-23m, 24-59m, 5-14y, 15-29y, 30-44y, 45+
linelist_cleaned$age_group <- age_categories(linelist_cleaned$age_years, 
                                             breakers = c(0, 0.5, 1, 2, 5,
                                                          15, 30, 45))
...
#   rename("Age group (months or years)" = age_group, 
...
plot_age_pyramid(linelist_cleaned, 
                 age_group = "age_group", 
                 split_by = "sex") + 
  labs(y = "Cases (n)", x = "Age group (months or years)") + # change axis  labels (nb. x/y flip)
...
Of the patients, `r fmt_count(linelist_cleaned, patient_facility_type == "Outpatient")` were seen as outpatients and `r fmt_count(linelist_cleaned, patient_facility_type == "Inpatient")` were seen as inpatients. For inpatients, the median number of days admitted was `r median(linelist_cleaned$obs_days)`, 
with a range between `r min(linelist_cleaned$obs_days)` and `r max(linelist_cleaned$obs_days)` days.


Cases by illness severity at admission
...
`r fmt_count(linelist_cleaned, previously_vaccinated == "Yes - vaccination card/ink" | previously_vaccinated == "Yes - verbal")` reported previous vaccination, with `r fmt_count(linelist_cleaned, previously_vaccinated == "Yes - vaccination card/ink")` showing vaccination cards / ink and `r fmt_count(linelist_cleaned, previously_vaccinated == "Yes - verbal"` with recall only. `r fmt_count(linelist_cleaned, previously_vaccinated == "No")` were unvaccinated and `r fmt_count(linelist_cleaned, previously_vaccinated == "Unsure")` were unsure.

#### Case fatality ratio 

To date, there have been `r fmt_count(linelist_cleaned, grepl("Dead", exit_status))` deaths, of which 
...
THIS WHOLE MORTALITY SECTION SHOULD BE DELETED UNLESS YOU ARE IN A CLOSED SETTING (e.g. refugee camp). 
...
```{r describe_admissions_by_epiweek_casedef}
# get counts and props of age groups by case definition 
# include column and row totals 
descriptive(linelist_cleaned, "epiweek", "case_def", coltotals = TRUE, rowtotals = TRUE) %>% 
  rename("Week" = epiweek, 
         "Confirmed cases (n)" = Confirmed_n, 
         "%" = Confirmed_prop, 
         "Probable cases (n)" = Probable_n, 
...
# include column and row totals 
descriptive(linelist_cleaned, "patient_origin", "exit_status", 
            coltotals = TRUE, rowtotals = TRUE) %>% 
