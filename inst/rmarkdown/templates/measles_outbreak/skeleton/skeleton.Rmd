...
THIS WHOLE MORTALITY SECTION SHOULD BE DELETED UNLESS YOU ARE IN A CLOSED SETTING (e.g. refugee camp) AND HAVE ALL MORTALITY IN YOUR LINE LIST. 
The assumptions don't hold in an open/community setting. It is very unlikely that your disease-specific line list includes all-cause mortality.
...
code chunk. 
This assumes that you are capturing every death in your population (unlikely for an outbreak-specific line list) and that your population remains stable over the time period of interest. 
...
We could then also look at admissions and exits by epiweek for operational purposes. 

```{r describe_admissions_by_epiweekf}
# get counts and props of age groups by case definition 
# include column and row totals 
descriptive(linelist_cleaned, "epiweek", 
...
# include column and row totals 
descriptive(linelist_cleaned, "epiweek", "exit_status", 
            coltotals = TRUE, rowtotals = TRUE) %>% 
  rename("Week" = "epiweek", 
