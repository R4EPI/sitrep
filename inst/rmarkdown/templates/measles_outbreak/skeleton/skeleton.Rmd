...
# library(excel.link)
# 
# linelist_raw <- xl.read.file(here("linelist.xlsx"),
#                              xl.sheet = "Sheet1",
...
# linelist_raw <- rio::import(here("linelist.xlsx"), range = "B2:J102")
...
# Stata data file ---------------------------------
# linelist_raw <- rio::import(here("linelist.dat"))
...
## on the measles dictionary. This will copy a rename command like the one above
## to your clipboard.
...
## Excel file
## to read in a specific sheet use "which"
# population_data <- rio::import(here("population.xlsx"), which = "Sheet1")
...
## uses proportions from the OCBA population denominators tool v1
## modify to fit your population or known parameters
...
## Excel file
## to read in a specific sheet use "which"
# lab_data <- rio::import(here("labresults.xlsx"), which = "Sheet1")
...
## OPTIONAL: if you want to expect certain variables,
...
linelist_cleaned %>%
  filter(patient_facility_type == "Inpatient") %>%
  case_fatality_rate_df(deaths = DIED, group = age_category, 
                        mergeCI = TRUE, add_total = TRUE) %>%
  tidyr::complete(age_category, 
                  fill = list(deaths = 0, population = 0, 
                              cfr = 0, ci = 0)) %>% # Ensure all levels are represented
...
tab_linelist(linelist_cleaned, epiweek,
             col_total = TRUE) %>% 
  select(-variable) %>%
  rename("Week" = value,
         "Cases (n)" = n,
