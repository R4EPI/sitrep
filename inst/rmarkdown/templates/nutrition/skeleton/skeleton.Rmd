...
study_data_cleaned <- study_data_cleaned %>%
  mutate(
    across(
      .cols = contains("q53"), # all variables that contain the word "q53"
      .fns = ~factor(replace_na(., "")) # replace all missing values with "" and turn into a factor
      ) 
    )
...
study_data_cleaned <- study_data_cleaned %>% 
  ...
  mutate(
    across(
      .cols = vars(gam_whz, mam_whz, sam_whz), # select the indicator variables
