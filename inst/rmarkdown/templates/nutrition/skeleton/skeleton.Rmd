...
```{r}
# read in data
# you can use readxl to read in Excel files
survey_data <- readxl::read_xlsx("<your_data>")
...
weigth_for_height <- zscore_results %>% 
...
stratify_zscores(dplyr::bind_cols(survey_data, weigth_for_height), by = "sex", GAM, MAM, SAM) %>%
  knitr::kable()
```
...
cbind(survey_data, weigth_for_height) %>% 
  ...
  summarise(n = n(), 
            ...
            rel = flagged / n, 
            !!! sitrep:::splice_df(binom::binom.wilson(flagged, n), lower, upper)) %>% 
