...
study_data_cleaned <- study_data_cleaned %>% 
  mutate(
    underweight_waz = zwei < -2, 
    moderate_underweight_waz = zwei >= -3 & zwei < -2, 
    severe_underweight_waz = zwei < -3
  ) %>%
  # set flagged children to NA for each of the indicator variables
    # if the flag for this indicator is not missing and is 1 then set indicator vars to NA
    # else leave the indicator variable as it was
...
temp_data <- study_data_cleaned %>%
  gather("indicator", "value", zwfl, zlen, zwei) %>% 
  select(indicator, value)


ggplot(temp_data, aes(y = value, x = indicator)) + 
  geom_boxplot() + 
...
ggplot(study_data_cleaned, aes(y = zlen, x = age_group)) + 
  geom_boxplot() + 
  geom_hline(yintercept = 0)
...
temp_data <- study_data_cleaned %>%
  gather("indicator", "value", zwfl, zlen, zwei) %>%
  select(sex, indicator, value)


ggplot(temp_data, aes(y = value, x = indicator, fill = sex)) + 
  geom_boxplot() + 
  geom_hline(yintercept = 0)
```
...
weight_for_height <- zscore_results %>% 
  transmute(GAM = tidyr::replace_na(zwei < -2 | study_data_cleaned$oedema == "Yes", FALSE), 
            MAM = tidyr::replace_na(-3 <= zwei & zwei < -2),
            SAM = tidyr::replace_na(zwei < -3 |  study_data_cleaned$oedema == "Yes"))
```
...
totals <- c(length(study_data_cleaned$sex), sum(study_data_cleaned$sex == "F"), sum(study_data_cleaned$sex == "M"))

dplyr::bind_cols(study_data_cleaned, weight_for_height) %>%
  stratify_zscores(by = "sex", GAM, MAM, SAM) %>%
  mutate(value = paste(flagged," ", fmt_ci(rel, lower, upper))) %>% 
  select(stratum, sex, value) %>% 
  mutate(sex = recode(sex, .missing = "All", "F" = "Sex 1", "M" = "Sex 2")) %>% 
...
```{r}
muac <- study_data_cleaned %>% 
  transmute(GAM = tidyr::replace_na(muac_mm_left_arm < 125 | oedema == "Yes", FALSE), 
            MAM = tidyr::replace_na(muac_mm_left_arm < 125 & muac_mm_left_arm >= 115 &  oedema == "No"),
            SAM = tidyr::replace_na(muac_mm_left_arm < 115 |  oedema == "Yes"))
```
...
dplyr::bind_cols(study_data_cleaned, muac) %>%
  stratify_zscores(by = "sex", GAM, MAM, SAM) %>%
  mutate(value = paste(flagged," ", fmt_ci(rel, lower, upper))) %>% 
  select(stratum, sex, value) %>% 
...
dplyr::bind_cols(study_data_cleaned, chronic) %>%
