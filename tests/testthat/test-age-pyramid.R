...
apnp <- plot_age_pyramid(dat, age_group = AGE, facet = FALSE)
apg_stack <- plot_age_pyramid(dat, age_group = AGE, split_by = gender, facet = FALSE)
# missing data
...
test_that("plots appear the same", {
  ...
  vdiffr::expect_doppelganger("gender age pyramid, stacked" , apg_stack)
...
test_that("missing split data are removed before plotting", {
  dat$sex[69] <- NA
  expect_warning(plot_age_pyramid(dat, age_group = "AGE", na.rm = TRUE),
